...
| `cv/tailor.py` | Vacancy tailoring for a `CVDocument`, with the integrity checks described below. `tailor_cv_document(..., memories=)` takes the memories allowed on a CV as extra evidence: they may reword the profile text, a description or a bullet, and an entry may gain one bullet per memory its patch names in `memories`. A label the prompt did not hold, a wish or condition, or a memory already used by another entry backs no bullet, and the integrity check is unchanged, so a memory never adds an employer, title, date, school or skill item. |
